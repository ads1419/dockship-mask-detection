...
parser.add_argument(
    "--mc",
    default="./weights/trained-resnet34.pkl",
    type=str,
    help="Trained state_dict file path to open",
)

parser.add_argument(
    "--input", default="../input/", type=str, help="Dir to fetch input images"
)
parser.add_argument(
    "--output", default="../output/", type=str, help="Dir to save image results"
)
...
parser.add_argument(
    "--resize", default=0.75, type=float, help="Scaling factor for testing images"
)
...
def get_face_bboxes():
    ...
    print(
        "im_detect: [forward_pass_time: {:.4f}s misc: {:.4f}s] per image".format(
            _t[forward_pass_det].average_time, _t[misc_det].average_time
        )
    )
    return good_dets
...
if __name__ == "__main__":
    ...
    _t = {
        forward_pass_det: Timer(),
        forward_pass_cls: Timer(),
        misc_det: Timer(),
        misc_cls: Timer(),
    }
    ...
    for image in tqdm(list(good_dets.keys())):
        ...
        for box in good_dets[image]:
            ...
            xmin = max(box[0] - xpixels, 0.0)
            ymin = max(box[1] - ypixels, 0.0)
        ...
        for i, b in enumerate(good_dets[image]):
            ...
            if args.save_dets:
                f.write(
                    f"{cats[int(preds[i][0])]} {b[4]} {int(b[0])} {int(b[1])} {int(b[2])} {int(b[3])}\n"
                )
    ...
    print(
        f"im_classify: [forward_pass_time: {_t[forward_pass_cls].average_time:.4f}s misc: {_t[misc_cls].average_time:.4f}s] per image\n\n"
    )
    if args.save_image:
        print(f"IMAGES SAVED in {output_dir}")
    if args.save_dets:
        print(f"DETECTIONS SAVED in {str(det_path)}\n\n")
